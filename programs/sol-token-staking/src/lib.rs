...
pub mod staking {
    ...
    pub fn create_stake(ctx: Context<CreateStake>, amount: u64, end_date: u128) -> Result<()> {
        token::transfer(ctx.accounts.into(), amount)?;

        let stake = &mut ctx.accounts.stake;
        stake.staker = *ctx.accounts.staker.key;
        stake.amount = amount;
        // Can specify the end time here.
    ...
    pub fn end_stake(ctx: Context<EndStake>) -> Result<()> {
        ...
        if ctx.accounts.stake.end_time > Clock::get().unwrap().unix_timestamp as u128 {
            return Err(error!(StakingErrors::EndTimeNotReached));
        }
